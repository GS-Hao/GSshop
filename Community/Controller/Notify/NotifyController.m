...
- (void)addNotify:(NSString *)text {
    GSNotify *no = [AIDCoreDataManager getNotifyEntityByContent:text];
    if (no) {
        return;
    }
    NSArray *array = [AIDCoreDataManager getAllEntitys:[GSNotify class]];
    GSNotify *noti = [AIDCoreDataManager createEntity:[GSNotify class]];
    noti.id = (int)array.count;
    noti.content = text;
    noti.create_time = [NSString stringWithFormat:@"%@", [NSDate date]];
    noti.order = (int)array.count;
    noti.url = @"";
...
- (void)loadData:(BOOL)first{
    ...
    int userid = data?[data[@"user_id"] intValue]:0;
    NSArray *more = [AIDCoreDataManager getNotifyEntityByUserid:userid];
...
//    NSMutableDictionary *data=[userDefaults getToken:@"user_cookie"];
    ...
    [obj gets:params successBlock:^(id  _Nonnull responseObject) {
        NSMutableArray *more = [GSNotify mj_objectArrayWithKeyValuesArray:responseObject[@"list"]];
        [self.fields addObjectsFromArray:more];
...
//            [self.navigationController pushViewController:vc animated:YES];
    ...
    self.dataSource = [[TableViewDataSource alloc] initWithData:self.fields identifier:@"notify" cellBlock:^(NotifyCell *cell, GSNotify *field, NSIndexPath *indexPath){
        cell.field = field;
    }];
