...
-(void)loadData{
    ...
    int userid = [_cache[@"user_id"] intValue];
    [self.scrollView.mj_header endRefreshing];//结束刷新
    GSUser *info = [AIDCoreDataManager getUserEntityById:[_cache[@"user_id"] intValue]];
    [self headerView:info];//赋值
    
    NSArray *comment = [AIDCoreDataManager getCommentEntityByUid:userid];
    NSArray *fUsers = [AIDCoreDataManager getFollowUserEntityByUid:userid];
    
    self.fields2=@[@{@"id":@"0", @"title":@"获赞", @"count":[NSString stringWithFormat:@"%d", info.like]}, @{@"id":@"1", @"title":@"关注", @"count":[NSString stringWithFormat:@"%zd", fUsers.count]}, @{@"id":@"2", @"title":@"粉丝", @"count":[NSString stringWithFormat:@"%d", info.followers]}, @{@"id":@"3", @"title":@"作品", @"count":[NSString stringWithFormat:@"%zd", [comment count]]}];
...
//        [self.scrollView.mj_header endRefreshing];//结束刷新
//        GSUser *info = [GSUser mj_objectWithKeyValues:responseObject];
//        [self headerView:info];//赋值
//        self.fields2=@[@{@"id":@"0", @"title":@"获赞", @"count":[NSString stringWithFormat:@"%d", info.like]}, @{@"id":@"1", @"title":@"关注", @"count":[NSString stringWithFormat:@"%d", info.following]}, @{@"id":@"2", @"title":@"粉丝", @"count":[NSString stringWithFormat:@"%d", info.followers]}, @{@"id":@"3", @"title":@"作品", @"count":[NSString stringWithFormat:@"%d", info.posts]}];
//        //NSLog(@"self.field:%@", self.fields2);
//        [self.scrollView addSubview:self.shortcutView];
//        [self createSquares:[Shortcut mj_objectArrayWithKeyValuesArray:self.fields2]];
//        [self.scrollView addSubview:self.bannerView];
...
#pragma mark - 头部视图
-(MyHeaderView *)headerView:(GSUser *)field{
    if (_headerView==nil) {
...
-(BannerView *)bannerView{
    if (nil==_bannerView) {
        ...
        _bannerView.frame=CGRectMake(10, 220, CWScreenW-20, 1);
        NSString *f=nil;
        _bannerView.field=f;
    }
    return _bannerView;
