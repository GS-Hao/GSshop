...
-(void)doPublish{
    ...
    if (nil!=_cache) {
        ...
        HomeJoke *homejoke = [AIDCoreDataManager createEntity:[HomeJoke class]];
        NSArray *array = [AIDCoreDataManager getAllEntitys:[HomeJoke class]];
        GSUser *user = [AIDCoreDataManager getUserEntityById:[_cache[@"user_id"] intValue]];
        
        homejoke.avatar = @"";
        homejoke.cellHeight = 0;
        homejoke.content = _params[@"content"];
        homejoke.id = 1000+(int)array.count;
        homejoke.image = @"";
        homejoke.images = _params[@"images"];
        homejoke.is_favorite = 0;
        homejoke.is_follow = 0;
        homejoke.is_like = 0;
        homejoke.nickname = user.nickname;
        homejoke.type = [_params[@"type"] intValue];
        homejoke.user_id = [_params[@"user_id"] intValue];
        homejoke.video = _params[@"video"];
        homejoke.order = (int)array.count;
...
-(UITextView *)textView{
    if (nil==_textView) {
        _textView=[[UITextView alloc] initWithFrame:CGRectMake(10, 110, CWScreenW-20, 30)];
