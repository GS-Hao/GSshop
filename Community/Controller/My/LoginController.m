...
- (void)viewDidLoad {
    ...
    if (!_loginBtn.enabled) _loginBtn.backgroundColor = [bgColor colorWithAlphaComponent:0.4];
}
//文本框内容改变时调用
-(void)textChange{
    _loginBtn.enabled=_mobileText.text.length && _passwdText.text.length;
    if (_loginBtn.enabled){
        _loginBtn.backgroundColor = bgColor;
    }else _loginBtn.backgroundColor = [bgColor colorWithAlphaComponent:0.4];
...
- (void)mimaLogin {
    GSUser *user = [AIDCoreDataManager getUserEntityByMobile:self.mobileText.text];
    NSArray *array = [AIDCoreDataManager getAllEntitys:[GSUser class]];
    if (!user) {
        user = [AIDCoreDataManager createEntity:[GSUser class]];
        user.mobile = self.mobileText.text;
        user.passwd = self.passwdText.text;
        user.avatar = @"avatar/default/9.jpg";
        user.birthday = @"1990-01-01";
        user.city = @"东城区";
        user.id = 100+(int)[array count];
        user.status = 0;
        user.token = @"axx222";
        user.quotes = @"Sadsds";
        user.nickname = @"abc";
    ...
    if ([user.mobile isEqualToString:self.mobileText.text]&&[user.passwd isEqualToString:self.passwdText.text]) {
        //NSLog(@"user:%@", user);
        ...
        user_cookie[@"user_id"] = [NSString stringWithFormat:@"%d", user.id];
...
-(void) doLogin:(UIButton *)btn {
    ...
    if ([self validatePhoneNumber:self.mobileText.text]) {
        ...
        [obj mobileLogin:params successBlock:^(id  _Nonnull responseObject) {
            if ([responseObject[@"err"] isEqual:@0]) {
                GSUser *user = [AIDCoreDataManager createEntity:[GSUser class]];
                ...
                user.id = [[dic objectForKey:@"id"] intValue];
                user.status = [[dic objectForKey:@"status"] intValue];
                user.token = [dic objectForKey:@"token"];
                user.quotes = [dic objectForKey:@"quotes"];
                user.nickname = [dic objectForKey:@"nickname"];
                user.province = [dic objectForKey:@"province"];
                user.like = [[dic objectForKey:@"stat"][0][@"count"] intValue];
                user.following = [[dic objectForKey:@"stat"][1][@"count"] intValue];
                user.followers = [[dic objectForKey:@"stat"][2][@"count"] intValue];
                user.posts = [[dic objectForKey:@"stat"][3][@"count"] intValue];
...
-(void) btnTouchDown:(UIButton *)btn {
    btn.backgroundColor = [bgColor colorWithAlphaComponent:0.4];
}
