...
//    }];
    ...
    HomeJoke *field = [AIDCoreDataManager getHomeJokeEntityById:(int)self.id];
    //头部
...
-(void)createContent:(HomeJoke *)field{
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GSComment *field=self.fields[indexPath.row];
    cell.field = field;
    //取消选择cell颜色
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    if (![textField.text isEqualToString:@""]) {
        ...
        if (nil!=data) {
            ...
            HomeJoke *joke = [AIDCoreDataManager getHomeJokeEntityById:(int)self.id];
            GSUser *user = [AIDCoreDataManager getUserEntityById:[[NSString stringWithFormat:@"%@", data[@"user_id"]] intValue]];
            NSArray *array = [AIDCoreDataManager getAllEntitys:[GSUser class]];
            GSComment *com = [AIDCoreDataManager createEntity:[GSComment class]];
            NSInteger count = [[AIDCoreDataManager getAllEntitys:[GSComment class]] count];
            
            com.avatar = joke.avatar;
            com.cellHeight = joke.cellHeight;
            com.content = textField.text?:@"";
            com.id = 100+(int)count;
            com.order = (int)count;
            com.image = joke.image;
            com.images = joke.images;
            com.joke_content = joke.content;
            com.joke_id = joke.id;
            com.nickname = user.nickname;
            com.topic_id = joke.topic_id;
            com.type = joke.type;
            com.user_id = user.id;
            com.video = joke.video;
...
- (void)loadData:(BOOL)first{
    ...
    NSArray *more = [AIDCoreDataManager getCommentEntityByJid:(int)self.id];
