...
- (void)loadData:(BOOL)first{
    ...
    [obj gets:params successBlock:^(id  _Nonnull responseObject) {
        ...
        NSString *path = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%@%@", @"joke", [NSString stringWithFormat:@"%zd", self.index]] ofType:@"json"];
        ...
        for (int i=0; i<[array count]; i++) {
            ...
            HomeJoke *homejoke = [AIDCoreDataManager getHomeJokeEntityById:[[item objectForKey:@"id"] intValue]];
            
            if (!homejoke) {
                homejoke = [AIDCoreDataManager createEntity:[HomeJoke class]];
            }
            
            homejoke.avatar = [item objectForKey:@"avatar"];
            homejoke.cellHeight = [[item objectForKey:@"cellHeight"] floatValue];
            homejoke.content = [item objectForKey:@"content"];
            homejoke.id = [[item objectForKey:@"id"] intValue];
            homejoke.image = [item objectForKey:@"image"];
            homejoke.images = [item objectForKey:@"images"];
            homejoke.is_favorite = [[item objectForKey:@"is_favorite"] intValue];
            homejoke.is_follow = [[item objectForKey:@"is_follow"] intValue];
            homejoke.is_like = [[item objectForKey:@"is_like"] intValue];
            homejoke.nickname = [item objectForKey:@"nickname"];
            homejoke.quotes = [item objectForKey:@"quotes"];
            homejoke.topic_id = [[item objectForKey:@"topic_id"] intValue];
            homejoke.topic_name = [item objectForKey:@"topic_name"];
            homejoke.type = [[item objectForKey:@"type"] intValue];
            homejoke.user_id = [[item objectForKey:@"user_id"] intValue];
            homejoke.video = [item objectForKey:@"video"];
            homejoke.order = i;
            [[AppDelegate sharedInstance] saveContext];
        }
        
        NSArray *result = [AIDCoreDataManager getHomeJokeEntityByType:(int)self.index];
        
        [self.fields addObjectsFromArray:result];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    HomeJoke *field=self.fields[indexPath.row];
    cell.field=field;
    return cell;
}
//点击某个cell时调用
