...
-(instancetype)init{
    if (self =[super init]) {
        ...
        //关注按钮
        //[_follow setBackgroundColor:bgColor];
...
-(void)setField:(HomeJoke *)field{
    ...
    [_follow setTitleColor:bgColor forState:UIControlStateNormal];
    [_follow.layer setBorderColor:bgColor.CGColor];
    if (field.is_follow) {
...
-(void)doFollow:(UIButton *)btn{
    ...
    if (nil!=data) {
        GSFollowUser *fUser = [AIDCoreDataManager createEntity:[GSFollowUser class]];
        NSArray *fUsers = [AIDCoreDataManager getAllEntitys:[GSFollowUser class]];
        fUser.avatar = _field.avatar;
        fUser.id = 200+(int)fUsers.count;
        fUser.nickname = _field.nickname;
        fUser.to_user_id = _field.user_id;
        fUser.topic_id = 0;
        fUser.type = 1;
        fUser.user_id = [data[@"user_id"] intValue];
        ...
        params[@"to_uid"] = [NSString stringWithFormat:@"%d", _field.user_id];
        [obj run:params method:method successBlock:^(id  _Nonnull responseObject) {
            ...
            if (self.field.is_follow) {//取反
                ...
                [self.follow setTitleColor:bgColor forState:UIControlStateNormal];
                [self.follow.layer setBorderColor:bgColor.CGColor];
            }else{
            ...
            //self.follow.backgroundColor = [bgColor colorWithAlphaComponent:0.4];
