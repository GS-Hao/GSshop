...
//        [_avatar addGestureRecognizer:ges];
        ...
        [_topic setTitleColor:bgColor forState:UIControlStateNormal];
        [_topic.layer setBorderColor:bgColor.CGColor];
        //设置圆角的大小
...
-(NSMutableArray *)_buildGridLayoutArgs:(HomeJoke *)field{
    NSMutableArray *arr=[[NSMutableArray alloc] init];
    if (![field.images isEqualToString:@""]) {
        //图集
        NSArray *images= [field.images componentsSeparatedByString:@","];
        for (int i=0; i<images.count; i++) {
            NSDictionary *dict=[NSDictionary dictionaryWithObjects:@[[NSString stringWithFormat:@"%d", field.type], images[i], images[i]] forKeys:@[@"type", @"image", @"original"]];
            arr[i]=dict;
        }
    }else if(![field.image isEqualToString:@""]){
        //视频头图
        NSDictionary *dict=[NSDictionary dictionaryWithObjects:@[[NSString stringWithFormat:@"%d", field.type], field.image, field.video] forKeys:@[@"type", @"image", @"original"]];
        arr[0]=dict;
    }
    return arr;
}
...
-(void)doLike:(UIButton *)btn{
    ...
    if (nil!=data) {
        ...
        GSLike *like = [AIDCoreDataManager getLikeEntityById:[data[@"user_id"] intValue] AndJokeid:_field.id];
        HomeJoke *joke = [AIDCoreDataManager getHomeJokeEntityById:_field.id];
        NSArray *array = [AIDCoreDataManager getAllEntitys:[GSLike class]];
        if (!like) {
            like = [AIDCoreDataManager createEntity:[GSLike class]];
        }
        like.user_id = [data[@"user_id"] intValue];
        like.joke_id = _field.id;
        like.order = (int)array.count;
        like.id = 100+(int)array.count;
        
        like.cellHeight = joke.cellHeight;
        like.content = joke.content;
        like.image = joke.image;
        like.images = joke.images;
        like.topic_id = joke.topic_id;
        like.type = joke.type;
        like.video = joke.video;
...
//        params[@"joke_id"] = [NSString stringWithFormat:@"%d", _field.id];
...
-(void)doFavorite:(UIButton *)btn{
    ...
    if (nil!=data) {
        ...
        GSFavorite *like = [AIDCoreDataManager getFavoriteEntityById:[data[@"user_id"] intValue] AndJokeid:_field.id];
        HomeJoke *joke = [AIDCoreDataManager getHomeJokeEntityById:_field.id];
        NSArray *array = [AIDCoreDataManager getAllEntitys:[GSFavorite class]];
        if (!like) {
            like = [AIDCoreDataManager createEntity:[GSFavorite class]];
        }
        like.user_id = [data[@"user_id"] intValue];
        like.joke_id = _field.id;
        like.order = (int)array.count;
