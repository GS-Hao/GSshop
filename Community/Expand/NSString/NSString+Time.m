...
+ (NSString *)ArrayOrDicZhuanJsonWithData:(id)Data;
{
	...
	NSData *data_json =[NSJSONSerialization dataWithJSONObject:Data options:NSJSONWritingPrettyPrinted error:nil];

    NSString *jsonStr =[[NSString alloc]initWithData:data_json encoding:NSUTF8StringEncoding];
...

{
	...
	if (GSisKindOfClass(self, NSDate)) {
		date = self;
	}else{
		NSTimeInterval timeva = [self doubleValue];
		NSDate *detaildate=[NSDate dateWithTimeIntervalSince1970:timeva];
		NSDateFormatter *format = [[NSDateFormatter alloc]init];
		[format setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
		NSString *selfStr = [format stringFromDate:detaildate];
		date = [format dateFromString:selfStr];
	}
	NSCalendar *calendar = [NSCalendar currentCalendar];
